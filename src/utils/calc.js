...
export const medianFind = (data, allData, key) => {
  let medianList = data.map((item) => {
    ...
    if (item.count % 2) {
      ...
      //even
      let index1 = parseInt(nthIndex + item.count / 2);
      let index2 = parseInt(nthIndex + (item.count / 2 + 1));

      let value1 = allData[index1 - 1][key];
      let value2 = allData[index2 - 1][key];
      let median = (value1 + value2) / 2;
      return { ...item, median: Math.round(median * 1000) / 1000 };
    }
  });
  return medianList;
};
